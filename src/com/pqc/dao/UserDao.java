...
public class UserDao {
	...
	public User checkPhone(String phone) {
		User user = null;
		try {
			user = qRunner.query("select * from user where phone=?",
	...
	public int registUser(User user) {
		int row = 0;
		try {
			row = qRunner.update("insert into user (uname,gender,phone,area,username,password,photo,create_time) values(?,?,?,?,?,?,?,?)", 
					user.getUname(), user.getGender(),user.getPhone(),user.getArea(),user.getUsername(),user.getPassword(),user.getPhoto(),user.getCreate_time());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
