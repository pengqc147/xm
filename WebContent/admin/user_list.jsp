		        ...
		        <c:forEach items="${users }" var="user" varStatus="i">
		         	<tr style="text-align : center">
				        ...
				        <td>
				        	...
				        	<c:if test="${user.manager == 0 }"><font color="green">管理员</font></c:if>
				        </td>
				        <td>${user.username }</td>
				       	<td>
				       		<img  src="<%=imgpath %>${user.photo }" width="80" height="50" />
				       	</td>
				       	<td>
				       	...
				       	<td>
				       		<c:if test="${user.manager == 1 }">
				       			<a style="color: green;" href="<%=basePath %>user?func=updateManage&uid=${user.uid}&manager=0" >设为管理员</a>
				       		</c:if>
				        	<c:if test="${user.manager == 0 }">
				       			<a style="color: red;" href="<%=basePath %>user?func=updateManage&uid=${user.uid}&manager=1" >撤销管理员</a>
				       		</c:if>
				       	</td>
			        </tr>
		        </c:forEach>
